...
func TestBelmanFordAlgorithm(t *testing.T) {
	tests := []struct {
		in_start  string
		in_end    string
		in_graph  []Graph
	...
	for _, test := range tests {
		actual_route, actual_cost := BellmanFordAlgorithm(test.in_start, test.in_end, test.in_graph)
		if !reflect.DeepEqual(actual_route, test.out_route) || actual_cost != test.out_cost {
			t.Errorf("BellmanFordAlgorithm(%v, %v, %v) = %v, %v want %v, %v", test.in_start, test.in_end, test.in_graph, actual_route, actual_cost, test.out_route, test.out_cost)
		}
	}
}
